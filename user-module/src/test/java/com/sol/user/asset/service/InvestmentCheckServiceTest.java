...
class InvestmentCheckServiceTest {
    ...
    void 자산이_없으면_비율0_역할빈리스트_성장블록null() {
        stubSnapshot(zeroBreakdown(), Map.of());
    ...
    void 자산을_4역할로_분해하고_비율합은_100() {
        ...
        stubSnapshot(new AssetBreakdown(
                ...
                Map.of("삼성전자", 20_000_000L, "SK하이닉스", 10_000_000L));
    ...
    void 성장블록_종목쏠림_최대비중과_레벨을_계산한다() {
        ...
        stubSnapshot(stockOnlyBreakdown(40_000_000), Map.of(
                "삼성전자", 12_000_000L, "SK하이닉스", 10_000_000L,
    ...
    void 단일종목만_보유하면_쏠림_높음() {
        stubSnapshot(stockOnlyBreakdown(30_000_000), Map.of("삼성전자", 30_000_000L));
    ...
    /** breakdown + 종목명→평가액 맵으로 STOCK 보유 + product 배치 응답을 함께 가진 스냅샷을 스텁한다. */
    private void stubSnapshot(AssetBreakdown breakdown, Map<String, Long> byName) {
        ...
        when(assetAggregator.aggregateSnapshot(USER_ID))
                .thenReturn(new AssetAggregator.AssetSnapshot(breakdown, List.of(), holdings, products));
    }
    ...
    private HoldingWithProduct holding(long productId, long eval) {
        ...
        when(holding.getEvaluationAmount()).thenReturn(BigDecimal.valueOf(eval));
        return holding;
    }
